...
- **Онбординг** (`/`) — первый запуск: имя (необязательно), пол,
  возраст, рост, вес, уровень активности и цель; завершается экраном
  «Готово» с переходом в приложение (профиль сохраняется локально).
...
- **Акцент** — один узнаваемый изумрудно-зелёный цвет (`--primary`) для
  ...
  мягкие цветовые различия (синий / янтарный / фиолетовый) и
  выводятся единой полосой с лёгкими цветными подложками.
- **Поверхности** — светлая лестница: фон → карточка (заливная,
  `rounded-3xl bg-card` с мягкой тенью) → подложки-подсветки →
  hover-состояния; тонкие разделители только в длинных списках.
- **Еда как еда** — иконки категорий в строках дневника и чипсах
  категорий, «пищевые» иконки приёмов (завтрак/салат/ужин/перекус),
  пустые приёмы приглашают: «Пока ничего нет» + акцентная кнопка «+».
- **Темы** — «Системная» (по умолчанию), «Светлая» и «Тёмная» в
  Настройках; смена происходит с мягким переходом цветов, системная
  тема отслеживается в реальном времени (ключ
  `nutritrack-theme`, старые значения «light»/«dark» совместимы).
- **Запуск приложения** — короткая (≈1,3 с) брендированная
  последовательность: логотип → приветствие по времени суток
  («Доброе утро, Игорь 👋») → спокойная фраза → плавный вход в
  «Сегодня»; отключается при prefers-reduced-motion и после
  онбординга.
- **Движение** — кольцо калорий «наполняется», число и полосы БЖУ
  анимируются при открытии и изменениях; строки дневника появляются и
  исчезают с анимацией; переходы между разделами ~200 мс; всё
  уважает `prefers-reduced-motion` и построено на CSS/rAF без
  сторонних библиотек.
